...
class Database
{
    ...
    private function connect()
    {
        if (!$this->handler) {
            ...
            try {
                ...
                throw new \Riff\Exception\Exception('Riff could not connect to the database');
            }
        }
    }
    ...
    public function delete($table, $where)
    {
        ...
        } catch (\Riff\Exception\Exception $e) {
            throw new \Riff\Exception\Exception('Could not delete from ' . $table);
        }
    }
    ...
    public function execute($query)
    {
        ...
        $query = (is_string($query)) ? new Query($query) : $query;

        $statement = $this->handler->prepare($query->fetchQuery());

        // If the statement failed
        if ($statement === false) throw new \Riff\Exception\Exception('Something went wrong preparing query "' . $query . '"');
        ...
        try {
            ...
            throw new \Riff\Exception\Exception($e->getMessage());
        }
    }
    ...
    public function insert($table, $values)
    {
        ...
        if (!is_array($values)) throw new \Riff\Exception\Exception('No values to insert');


        $sql = "INSERT INTO :table (";

        foreach ($values as $column => $value) {
            $sql .= \Riff\Filter\Filter::sanitize($column) . ',';
        }
        ...
        $query = new Query($sql, $values, array('table' => $table));

        try {
            $this->execute($query);
            return $this->handler->lastInsertId();
        } catch (\Riff\Exception\Exception $e) {
            throw new \Riff\Exception\Exception('Could not insert data into ' . $table);
        }
    }
    ...
    public function select($subject, $table, $where = null, $limit = null, $orderBy = null, $groupBy = null)
    {
        ...
        // Start the query
        $query = new Query('SELECT :subject FROM :table');
    ...
    public function update($table, $values, $where)
    {
        ...
        foreach ($values as $column => $value) {
            $sql .= \Riff\Filter\Filter::sanitize((string) $column) . ' = ' . ':' . $column . ', '; 
        }
        ...
        try {
            ...
            throw new \Riff\Exception\Exception('Could not update ' . $table);
        }
    }
}