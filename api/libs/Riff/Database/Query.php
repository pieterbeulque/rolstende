...
class Query
{
    ...
    {
        if (!is_array($parameters)) throw new \Riff\Exception\Exception('Parameters is not an array');
    ...
    public function addWhere($where)
    {
        if (is_array($where)) {
            ...
            foreach ($where as $key => $value) {
                $this->where .= \Riff\Filter\Filter::sanitize((string) $key) . ' = :' . (string) $key . ' AND ';
            }

            $this->addPDOParameters($where);
            $this->where = rtrim($this->where, ' AND '); 
        } else if (is_string($where)) {
            $this->where = \Riff\Filter\Filter::sanitize($where);
        } else {
            throw new \Riff\Exception\Exception('WHERE-clause is not valid');
        }
    }
    ...
    public function fetchQuery()
    {
        ...
        foreach ($this->parameters as $parameter => $value) {
            $value = \Riff\Filter\Filter::sanitize((string) $value);
